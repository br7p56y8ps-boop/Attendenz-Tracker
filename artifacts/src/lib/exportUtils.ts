...
export async function generateExcelReport(options: ExportReportOptions) {
  ...
  if (academicItems.length > 0) {
    ...
    rows.push({
      ...
      'Attendance Status': getReportStatus({ name: 'Academic Summary', attended: overallAttended, total: overallTotal, plannedTotal: academicItems.reduce((sum, item) => sum + item.plannedTotal, 0), pct: overallPct, neededForTarget: '' }, targetPct).label,

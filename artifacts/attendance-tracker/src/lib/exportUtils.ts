...
async function handoffPDFForIOSPWA(doc: { output: (type: 'blob') => Blob }, filename: string): Promise<void> {
  ...
  if (!opened) {
    ...
    link.setAttribute('aria-label', `Open ${filename}`);
...
export async function generatePDFReport(options: ExportReportOptions) {
  ...
  if (isIOSStandalonePWA()) await handoffPDFForIOSPWA(doc, filename);

...
/* SGT Overlap Check vs Academic Timeline */
const timeOverlaps: any[] = [];
for (const r of rows) {
for (const c of findSubjectTimeConflicts([r.day], r.time, undefined, 'academic')) {
if (!c.exact) timeOverlaps.push({ day: c.day, time: c.time, subjects: c.subjects });
}
}
...
setConflictSheet(null);
const newSubject = {
name: finalSgtName, subjectType: 'allied' as const, parentName: 'Small Group Teaching',
plannedClasses: pc, days: rows.map(r => r.day).join(', '), time: rows[0]?.time || '',
schedules: rows.map(r => ({ day: r.day, start: r.start, end: r.end })),
startDate: sgtStartDate, endDate: sgtEndDate, clinicalSubject: clinicalSubjectName,
};
...
setSgtClinicalSubject(''); setSgtName(''); setSgtStartDate(''); setSgtEndDate(''); setSgtRows([newRow([])]);
...
setTimeout(() => {
const newTargetSlots = timetableRef.current[targetDay] || [];
const newIdx = newTargetSlots.findIndex(sl => canonicalizeTimeRange(sl.time) === time);
if (newIdx >= 0) {
const merged = Array.from(new Set([...newTargetSlots[newIdx].subjects, ...namesToMove.slice(1)]));
updatePresetTimetableSlot(targetDay, newIdx, time, merged, targetDay);
} else {
namesToMove.slice(1).forEach(name => addSubjectToSlot(targetDay, time, name));
}
}, 0);
}
}

...
import {
  ...
  Check, ChevronDown, ChevronRight, Edit2, SendToBack, Hash,
} from 'lucide-react';
...
interface ImportBundle {
  ...
  customWards?: Array<{ name: string; startDate: string; endDate: string; morningTime?: string; eveningTime?: string; vacationPeriods?: Array<{ start: string; end: string }> }>;
...
// ── Vacation Editor (collapsed by default) ──
  ...
  return (
    <div className="border border-border/50 rounded-xl overflow-hidden bg-background/40">
      <button type="button" onClick={() => setOpen(!open)} className="w-full flex items-center justify-between px-3 py-2 hover:bg-muted/20 transition-colors text-left">
        <span className="text-[11px] font-bold text-foreground">Vacation within Rotational Period</span>
...
// ── Subject Triage Card (collapsed; expand → Rename / Planned / Delete; Swap removed) ──
function SubjectTriageCard({
  name, isPreset, store, id, planned, canEditPlanned, canDelete,
  onDelete, opdRename, opdEditing, toggleEdit, updateRename, saveRename, onSavePlanned,
}: any) {
  ...
  return (
    <div className="bg-card border border-border/50 rounded-lg overflow-hidden">
      {isEditing ? (
        ...
        <>
          <button type="button" onClick={() => { setShowPlannedEditor(false); }} className="w-full flex items-center justify-between p-2.5 hover:bg-muted/20 transition-colors text-left" onDoubleClick={() => {}}>
            ...
            <span className="text-[10px] text-muted-foreground shrink-0">Planned: {planned ?? '—'}</span>
          </button>
          <div className="px-2.5 pb-2.5">
            <div className="grid grid-cols-3 gap-1.5">
              ...
              <button type="button" disabled={!canEditPlanned} onClick={() => { setPlannedDraft(String(planned ?? '')); setShowPlannedEditor(!showPlannedEditor); }}
                className={cn('h-9 rounded-lg border text-[11px] font-bold flex items-center justify-center gap-1', canEditPlanned ? 'border-border bg-background/70 text-foreground hover:bg-muted cursor-pointer' : 'border-border/40 bg-muted/10 text-muted-foreground/40 cursor-not-allowed')}>
                <Hash className="w-3 h-3" /> Planned
              </button>
            ...
            {showPlannedEditor && canEditPlanned && (
              <div className="mt-1.5 flex items-center gap-1.5">
                <input type="number" min={0} value={plannedDraft} onChange={e => setPlannedDraft(e.target.value)} className={cn(inputCls, 'h-8 text-xs flex-1')} autoFocus />
                <button type="button" onClick={() => { const v = parseInt(plannedDraft, 10); if (!isNaN(v) && v >= 0) { onSavePlanned(v); setShowPlannedEditor(false); } }} className="p-1.5 rounded-lg text-primary hover:bg-primary/10 cursor-pointer"><Check className="w-3.5 h-3.5" /></button>
                <button type="button" onClick={() => setShowPlannedEditor(false)} className="p-1.5 rounded-lg text-muted-foreground hover:bg-muted cursor-pointer"><X className="w-3.5 h-3.5" /></button>
              </div>
            )}
          </div>
        </>
...
export default function AddNew() {
  const {
    ...
    getPresetWardTotalPlanned, // added
  ...
  const [replaceConfirm, setReplaceConfirm] = useState(false); // added
  ...
  /* Resolve a display name back to its original preset name */
  ...
  const resolvePresetWardOriginalName = (displayName: string): string => {
    const all = WARD_SUBJECTS.map(w => w.name);
    return all.find(n => getPresetWardDisplayName(n) === displayName) ?? displayName;
  };
  ...
  /* SGT planned = scheduled weekdays within range, minus Fridays/Holidays/vacations */
  ...
  const HISTORY_KEY = 'att_manage_history';
    ...
    try { const stored = localStorage.getItem(HISTORY_KEY); if (stored) setHistoryEntries(JSON.parse(stored)); } catch {}
      ...
      /* Reset Add New form each time it closes */
        useEffect(() => {
        if (!moreOpen) {
  ...
  const saveAddSlot = () => {
    ...
    if (conflicts.length > 0) { setFormError(`Time overlaps with ${conflicts.map(c => c.subjects.join(', ')).join('; ')}.`); return; }
    ...
    if (subjectMode === 'preloaded') {
      ...
      if (ua) {
        ...
        // preset subject: directly update timetable
    ...
    setAddSlotOpen(false); showToast('Slot added.');
  };
  ...
  const commitSubjects = (items: any[]) => {
    try {
      ...
      if (academicItems.length > 0) recordHistory('Add Subject', { names: academicItems.map((i: any) => i.name) });
      ...
      setMoreOpen(false); // close immediately instead of timeout
    } catch { showToast('Failed to save — please try again.', 'err'); }
  };
  ...
  const saveSubject = () => {
    ...
    } catch { showToast('Failed to check/save — please try again.', 'err'); }
  };
  ...
  const commitWard = (name: string, start: string, end: string, morningTime: string, eveningTime: string, vacations: VacationPeriod[]) => {
    try {
      if (subjectMode === 'preloaded') {
        addPresetWardEntry({ start, end, ward: name, morningTime, eveningTime, addedByUser: true });
      } else {
        addCustomWard({ name, startDate: start, endDate: end, morningTime, eveningTime, vacationPeriods: vacations.map(v => ({ start: v.start, end: v.end })) });
      }
      recordHistory('Add Rotation', { name, start, end });
      setWardName(''); setWardStart(''); setWardEnd(''); setWardVacations([]);
      setFormError(null); showToast('Rotation added.'); setMoreOpen(false);
  ...
  const saveClinicalItem = () => {
    if (clinicalParentChoice === 'rotation') {
      ...
      // SGT duplicate check: only among SGT records, not clinical subjects
      ...
      // SGT time conflicts: check BOTH academic timetable and other SGT records
      ...
      const doSave = () => {
        ...
        setFormError(null); showToast(`SGT added with ${pc} planned classes.`); setMoreOpen(false);
      };
  ...
  const openEditSlot = (day: number, index: number) => {
    ...
    setSlotConflict(null); setEditError(null); setShowMoveForm(false);
  };

  const closeEditSlot = () => { setEditSlot(null); setSelectedSubjects([]); setSlotRemove(null); setSlotRemoveConfirm(false); setSlotConflict(null); setEditError(null); setShowMoveForm(false); };
  ...
  const updateSubjectSchedule = (name: string, oldDay: number, newDay: number, oldStart: string, oldEnd: string, newStart: string, newEnd: string) => {
    const ua = userAddedSubjects.find(u => u.name.toLowerCase() === name.toLowerCase() && !isSGTRecord(u));
    if (!ua) return;
    ...
    const updated = filtered.some(s => s.day === newAbbr && s.start === newStart && s.end === newEnd) ? filtered : [...filtered, newEntry];
    updateUserAddedSubject(ua.id, { schedules: updated, days: updated.map(s => s.day).join(', ') } as any);
  };
  ...
  const applyMove = (targetIds: string[], targetDay: number, time: string) => {
    ...
    const currentDay = editSlot.day; const currentIndex = editSlot.index;
    ...
    const isSameSlot = targetDay === currentDay && time === canonicalizeTimeRange(sourceSlot.time);
      if (!isSameSlot) {
    ...
    if (existingIndex >= 0) {
      const merged = Array.from(new Set([...targetSlots[existingIndex].subjects, ...namesToMove]));
        updatePresetTimetableSlot(targetDay, existingIndex, time, merged, targetDay);
    } else {
        addSubjectToSlot(targetDay, time, namesToMove[0]);
        if (namesToMove.length > 1) {
        const nt = timetableRef.current[targetDay] || [];
        const ni = nt.findIndex(sl => canonicalizeTimeRange(sl.time) === time);
        if (ni >= 0) updatePresetTimetableSlot(targetDay, ni, time, Array.from(new Set([...nt[ni].subjects, ...namesToMove.slice(1)])), targetDay);
      }
    }
      if (sourceSubjects.length === 0) updatePresetTimetableSlot(currentDay, currentIndex, sourceSlot.time, [], currentDay);
      else updatePresetTimetableSlot(currentDay, currentIndex, sourceSlot.time, sourceSubjects, currentDay);
    }

    if (sourceSubjects.length === 0) updatePresetTimetableSlot(currentDay, currentIndex, sourceSlot.time, [], currentDay);
    else updatePresetTimetableSlot(currentDay, currentIndex, sourceSlot.time, sourceSubjects, currentDay);

    // Update scheduled rows for user-added subjects
    namesToMove.forEach(name => updateSubjectSchedule(name, currentDay, targetDay, oldStart, oldEnd, slotMoveStart, slotMoveEnd));

    // Planned class updates are intentionally not applied here (handled in Triage)
    const remaining = editSlot.subjects.filter(s => !targetIds.includes(s.id));
    if (remaining.length === 0) setEditSlot(null);
    else setEditSlot(prev => prev ? { ...prev, subjects: remaining.map(s => ({ ...s, id: genId('sel') })), targetDay } : null);
    setSelectedSubjects([]); setShowMoveForm(false); setSlotConflict(null);
    recordHistory('Move Subjects', { names: namesToMove, fromDay: currentDay, fromTime: sourceSlot.time, toDay: targetDay, toTime: time });
  ...
  const doMoveSubjects = (targetIdsOverride?: string[]) => {
    ...
    const targetDay = slotMoveTargetDay; const time = canonicalTimeRange(slotMoveStart, slotMoveEnd);
    const sourceTime = canonicalTimeRange(editSlot.startTime, editSlot.endTime);
    const isSameDay = targetDay === editSlot.day; const isSameTime = time === sourceTime;
    const targetSlots = timetableRef.current[targetDay] || [];
    const conflictSlot = targetSlots.find((sl, idx) => {
      if (isSameDay && idx === editSlot.index) return false;
      if (sl.type === 'ward' || sl.type === 'ward_replacement') return false;
      const pa = parseRangeToMinutes(sl.time); const pb = parseRangeToMinutes(time);
      return !!pa && !!pb && pa.start < pb.end && pb.start < pa.end;
    });
    if (conflictSlot && !isSameTime) {
      setSlotConflict({ messages: [`${DAY_ABBRS[targetDay]} already has a slot at ${conflictSlot.time} with ${conflictSlot.subjects.join(', ')}.`], onConfirm: () => { setSlotConflict(null); applyMove(targetIds, targetDay, time); } });
      return;
    }
  ...
  const confirmSlotRemove = () => {
    ...
    try {
      ...
      if (slot) {
        ...
        const ua = userAddedSubjects.find(u => u.name.toLowerCase() === slotRemove.subject.toLowerCase() && !isSGTRecord(u));
        if (ua) {
          const filtered = (ua.schedules || []).filter(s => !(s.day === DAY_ABBRS[slotRemove.day] && s.start === slotRemove.start && s.end === slotRemove.end));
  ...
  const openEditSubject = (store: 'userAdded' | 'custom', id: string) => {
    ...
    if (item.schedules && item.schedules.length) {
      // Detect schedule shape
      const first = item.schedules[0] as any;
      if ('start' in first && 'end' in first) {
        // userAdded shape
        rows = item.schedules.map(s => ({ id: genId('row'), day: s.day, startTime: s.start || '', endTime: s.end || '' }));
      } else {
        // custom shape: schedule has time
        rows = item.schedules.map(s => {
          const { start, end } = splitRange((s as any).time || '');
  ...
  const saveEditSubject = () => {
    ...
    try {
      if (editSubject.subjectType === 'allied-parent') {
        // duplicate check for allied-parent rename
        if (isSubjectNameTaken(editSubject.name, editSubject.originalName, 'academic')) {
          setEditError(`"${editSubject.name}" already exists.`); return;
        }
        ...
        if (editSubject.subjectType === 'allied' && editSubject.parentName !== 'Small Group Teaching') {
          ...
          if (editSubject.originalName.toLowerCase() === oldClinical.toLowerCase() &&
              editSubject.clinicalSubject !== oldClinical &&
              editSubject.name === editSubject.originalName) {
            patch.name = editSubject.clinicalSubject;
          }
        }

        // Duplicate checks for non-parent subjects
        if (isSGT) {
          const sgtList = editSubject.store === 'userAdded' ? userAddedSubjects : customSubjects;
          const existingSGT = sgtList.find(s => isSGTRecord(s) && s.id !== editSubject.id && s.name.toLowerCase() === patch.name.toLowerCase());
          if (existingSGT) { setEditError(`An SGT subject named "${patch.name}" already exists.`); return; }
        } else {
          if (isSubjectNameTaken(patch.name, editSubject.originalName, 'academic')) {
            setEditError(`"${patch.name}" already exists.`); return;
          }
        }
  ...
  const saveEditWard = () => {
    ...
    try {
      ...
      if (editWard.store === 'preset') {
        // Do not mutate schedule entry's ward name; use display rename if changed
        ...
        updateCustomWard(editWard.id!, {
          ...
          morningTime,
          eveningTime,
          vacationPeriods: (editWard.vacationPeriods || []).map(v => ({ start: v.start, end: v.end })),
        });
      }
  ...
  /* ── Triage ── */
  const openOpd = () => { setOpdRename({}); setOpdEditing({}); if (subjectMode === 'preloaded') setTriageTop('preset'); else setTriageTop('added'); setOpdOpen(true); };
  ...
  /* Display-map-only for presets; attendance rename only for added/custom */
  const saveOpdRename = (id: string, store: 'userAdded' | 'custom', currentName: string) => {
    ...
    const isPresetAcademic = CATEGORIES.flatMap(c => c.subjects).some(s => s.name === id) || INTEGRATED_SUBJECTS.some(s => s.name === id);
    ...
    if (isPresetAcademic) {
      if (isSubjectNameTaken(newName, currentName, 'academic')) {
        showToast(`"${newName}" already exists.`, 'err'); return;
      }
      setPresetSubjectRename(id, newName);
    ...
    if (isPresetClinical) {
      if (isWardNameTaken(newName, currentName)) {
        showToast(`"${newName}" already exists.`, 'err'); return;
      }
      renamePresetWard(id, newName);
      showToast(`Renamed to "${newName}".`);
      toggleOpdEdit(id);
      return;
    }

    const customWardTarget = customWards.find(w => w.id === id);
      if (customWardTarget) {
      if (isWardNameTaken(newName, customWardTarget.name)) { showToast(`"${newName}" already exists.`, 'err'); return; }
          renameWardData(customWardTarget.name, newName);
          updateCustomWard(id, { name: newName });
          showToast(`Renamed to "${newName}".`);
          toggleOpdEdit(id);
      return;
      }
      ...
      // SGT duplicates allowed with clinical subjects, but not among SGT
      const sgtList = store === 'userAdded' ? userAddedSubjects : customSubjects;
      const existingSGT = sgtList.find(s => isSGTRecord(s) && s.id !== id && s.name.toLowerCase() === newName.toLowerCase());
      if (existingSGT) { showToast(`An SGT subject named "${newName}" already exists.`, 'err'); return; }
      if (store === 'userAdded') updateUserAddedSubject(id, { name: newName });
      else updateCustomSubject(id, { name: newName });
      showToast(`Renamed to "${newName}".`);
      toggleOpdEdit(id);
      return;
    }
  ...
  /* Triage Planned editor */
  const saveTriagePlanned = (id: string, store: 'userAdded' | 'custom', value: number) => {
    const isPresetAcademic = CATEGORIES.flatMap(c => c.subjects).some(s => s.name === id) || INTEGRATED_SUBJECTS.some(s => s.name === id);
    if (isPresetAcademic) {
      updatePresetSubjectTotal(id, value);
      recordHistory('Edit Planned', { name: id, planned: value });
      showToast('Planned updated.');
      return;
    }
    const target = store === 'userAdded' ? userAddedSubjects.find(s => s.id === id) : customSubjects.find(s => s.id === id);
    if (!target) return;
    if (store === 'userAdded') updateUserAddedSubject(id, { plannedClasses: value });
    else updateCustomSubject(id, { plannedClasses: value });
    recordHistory('Edit Planned', { name: target.name, planned: value });
    showToast('Planned updated.');
  };
    ...
    setDeleteSheet({
      ...
      onConfirm: () => {
        ...
        if (target && isSGTRecord(target)) {
          ...
          // If allied-parent, also remove children's attendance
          if (target?.subjectType === 'allied-parent') {
            const kids = store === 'userAdded'
              ? userAddedSubjects.filter(x => x.subjectType === 'allied' && getEffectiveParentName(x)?.toLowerCase() === name.toLowerCase())
              : customSubjects.filter(x => x.subjectType === 'allied' && getEffectiveParentName(x)?.toLowerCase() === name.toLowerCase());
            for (const k of kids) removeSubjectData(k.name);
          }
        }
        setDeleteSheet(null); showToast(`Deleted "${name}".`);
      }
    });
  };
  ...
  const normTimeTo12 = (t: string): string => (is12h(t) ? t : to12h(parseRangeToMinutes(`${t}–${t}`)?.start || 0));
  ...
  const buildReport = (b: ImportBundle): ImportReport => {
    ...
    const slots = Object.values(b.presetTimetable || {}).reduce((acc: number, day: any) => {
  ...
  const applyMerge = () => {
    ...
    try {
      ...
      for (const w of b.customWards || []) {
        ...
        else addCustomWard({ name: w.name, startDate: w.startDate, endDate: w.endDate, morningTime: mt, eveningTime: et, vacationPeriods: w.vacationPeriods });
        wardsAdded++;
      }
      ...
      /* Merge the weekly timetable slots too */
      if (b.presetTimetable) {
        Object.entries(b.presetTimetable).forEach(([dayStr, slots]) => {
          ...
          for (const slot of slots as any[]) {
            ...
            const existingIdx = (timetableRef.current[day] || []).findIndex(s => canonicalizeTimeRange(s.time) === time);
            if (existingIdx >= 0) {
              const merged = Array.from(new Set([...(timetableRef.current[day][existingIdx].subjects || []), ...slot.subjects]));
  ...
  const applyReplace = () => {
    ...
    // Clear opposite mode data to avoid stale records
              ...
              <div className="flex items-center justify-between border-b border-border/40 pb-2">
                ...
                <span className="text-xs text-muted-foreground font-semibold">
                  {subjectMode === 'preloaded'
                    ? presetTimetable[selDay]?.filter(s => s.type !== 'ward' && s.type !== 'ward_replacement').length || 0
                    : customSubjects.filter(s => s.subjectType !== 'allied-parent' && parseDayList(s.days).includes(DAY_ABBRS[selDay])).length} Slots
                </span>
              </div>
              ...
              <>
                ...
                {clinicalParentChoice === 'rotation' ? (
                  <>
                    ...
                    {subjectMode === 'custom' && <VacationEditor vacations={wardVacations} onChange={setWardVacations} />}
                    {subjectMode === 'custom' && wardStart && wardEnd && wardEnd >= wardStart && (
                      <div className="bg-muted/30 p-2.5 rounded-xl flex justify-between items-center">
                        ...
                        <span className="text-xs font-extrabold text-foreground">{getCustomWardTotalPlanned(wardStart, wardEnd, wardVacations.map(v => ({ start: v.start, end: v.end })))}</span>
                      </div>
                    )}
                  </>
                ) : (
        ...
        <OverlayModal
          ...
          header={
            <>
              ...
              <p className="text-[10px] text-muted-foreground mt-1">Rename, change the parent, or reschedule. Planned is read-only — edit it in Subject Triage.</p>
            </>
          }
          footer={
        ...
        >
          {editWard && (
            <div className="p-4 sm:p-5 space-y-3.5">
              ...
              {editWard.store === 'custom' && (
                <VacationEditor
                  vacations={(editWard as any).vacationPeriods?.length ? (editWard as any).vacationPeriods.map((v: any, i: number) => ({ id: `wv_${i}`, ...v })) : []}
                  onChange={(v) => setEditWard({ ...editWard, vacationPeriods: v.map(x => ({ start: x.start, end: x.end })) } as any)}
                />
              )}
            </div>
          )}
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-muted-foreground mt-0.5">
                  {editSlot.multiSelectMode ? 'Select subject cards you want to reallocate/re-slot.' : 'Change time or day. Planned is read-only — edit it in Subject Triage.'}
                </p>
              </div>
              ...
              {slotConflict ? (
                ...
                <>
                  ...
                  <div className="bg-muted/30 p-2 rounded-lg">
                    ...
                    <p className="text-[10px] text-muted-foreground mt-1">Planned: {editSlot.subjects[0].planned} (read-only — edit in Subject Triage)</p>
                  </div>
                </>
              )}
            </div>
          ...
          <div className="p-4 sm:p-5 space-y-3.5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-muted-foreground">Rename, edit planned, or delete. Preset planned/wards are read-only where auto-calculated.</p>
              </div>
            ...
            <div className="space-y-1 h-[50vh] overflow-y-auto pr-1">
                  ...
                  {section === 'academic' ? (
                    <>
                      {CATEGORIES.flatMap(c => c.subjects).map(s => (
                        <SubjectTriageCard key={s.name} name={getPresetSubjectDisplayName(s.name)} isPreset={true} store="userAdded" id={s.name}
                          planned={getSubjectPlannedTotal(s.name)} canEditPlanned={true} canDelete={false}
                          onDelete={() => {}} opdRename={opdRename} opdEditing={opdEditing} toggleEdit={toggleOpdEdit} updateRename={updateOpdRename} saveRename={saveOpdRename}
                          onSavePlanned={(v: number) => saveTriagePlanned(s.name, 'preset', v)} />
                      ))}
                    ...
                    <>
                      {WARD_SUBJECTS.map(w => (
                        <SubjectTriageCard key={w.name} name={getPresetWardDisplayName(w.name)} isPreset={true} store="userAdded" id={w.name}
                          planned={getPresetWardTotalPlanned(w.name)} canEditPlanned={false} canDelete={false}
                          onDelete={() => {}} opdRename={opdRename} opdEditing={opdEditing} toggleEdit={toggleOpdEdit} updateRename={updateOpdRename} saveRename={saveOpdRename}
                          onSavePlanned={() => {}} />
                      ))}
                      {userAddedSubjects.filter(s => isSGTRecord(s)).map(s => (
                        <SubjectTriageCard key={s.id} name={s.name} isPreset={false} store="userAdded" id={s.id}
                          planned={s.plannedClasses} canEditPlanned={false} canDelete={true}
                          onDelete={() => deleteOpdSubject(s.id, 'userAdded', s.name)} opdRename={opdRename} opdEditing={opdEditing} toggleEdit={toggleOpdEdit} updateRename={updateOpdRename} saveRename={saveOpdRename}
                          onSavePlanned={() => {}} />
                      ))}
                    </>
                  )}
              ...
              {triageTop === 'added' && (
                <>
                  {section === 'academic' ? (
                    <>
                      {(subjectMode === 'preloaded' ? userAddedSubjects.filter(s => s.subjectType !== 'allied-parent' && !isSGTRecord(s)) : customSubjects.filter(s => s.subjectType !== 'allied-parent' && !isSGTRecord(s))).map(s => (
                        <SubjectTriageCard key={s.id} name={s.name} isPreset={false} store={subjectMode === 'preloaded' ? 'userAdded' : 'custom'} id={s.id}
                          planned={s.plannedClasses} canEditPlanned={true} canDelete={true}
                          onDelete={() => deleteOpdSubject(s.id, subjectMode === 'preloaded' ? 'userAdded' : 'custom', s.name)}
                          opdRename={opdRename} opdEditing={opdEditing} toggleEdit={toggleOpdEdit} updateRename={updateOpdRename} saveRename={saveOpdRename}
                          onSavePlanned={(v: number) => saveTriagePlanned(s.id, subjectMode === 'preloaded' ? 'userAdded' : 'custom', v)} />
                      ))}
                    </>
                  ) : (
                    <>
                      {customWards.map(w => (
                        <SubjectTriageCard key={w.id} name={w.name} isPreset={false} store="custom" id={w.id}
                          planned={getCustomWardTotalPlanned(w.startDate, w.endDate, w.vacationPeriods)} canEditPlanned={false} canDelete={true}
                          onDelete={() => deleteOpdWard(w.id, w.name)} opdRename={opdRename} opdEditing={opdEditing} toggleEdit={toggleOpdEdit} updateRename={updateOpdRename} saveRename={saveOpdRename}
                          onSavePlanned={() => {}} />
                      ))}
                      {(subjectMode === 'preloaded' ? userAddedSubjects.filter(s => isSGTRecord(s)) : customSubjects.filter(s => isSGTRecord(s))).map(s => (
                        <SubjectTriageCard key={s.id} name={s.name} isPreset={false} store={subjectMode === 'preloaded' ? 'userAdded' : 'custom'} id={s.id}
                          planned={s.plannedClasses} canEditPlanned={false} canDelete={true}
                          onDelete={() => deleteOpdSubject(s.id, subjectMode === 'preloaded' ? 'userAdded' : 'custom', s.name)}
                          opdRename={opdRename} opdEditing={opdEditing} toggleEdit={toggleOpdEdit} updateRename={updateOpdRename} saveRename={saveOpdRename}
              ...
              {((triageTop === 'preset' && subjectMode === 'preloaded' && section === 'academic' && CATEGORIES.flatMap(c => c.subjects).length === 0 && INTEGRATED_SUBJECTS.length === 0) ||
                ...
                (triageTop === 'added' && section === 'clinical' && customWards.length === 0 && (subjectMode === 'preloaded' ? userAddedSubjects : customSubjects).filter(s => isSGTRecord(s)).length === 0)) && (

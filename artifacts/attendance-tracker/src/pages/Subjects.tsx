...
import { CATEGORIES, WARD_SUBJECTS, getCurrentWard, getWardTotalPlanned } from '@/lib/constants';
...
export default function Subjects() {
  ...
  const [openCategories, setOpenCategories] = useState<Record<string, boolean>>({
    'Medicine & Allied': true, // Open by default
  });
            ...
            <div key={idx} className="flex flex-col gap-3">
              ...
              >
                <div className="flex justify-between items-center mb-3">
                  ...
                  {isOpen ? <ChevronUp className="text-muted-foreground w-5 h-5" /> : <ChevronDown className="text-muted-foreground w-5 h-5" />}
                </div>
                
                <div className="flex gap-4">
                  <div>
                    <div className="text-2xl font-bold" style={{ color: summary.percentage >= 75 ? 'var(--color-success)' : summary.percentage >= 65 ? 'var(--color-warning)' : 'var(--color-destructive)' }}>
                  ...
                  {WARD_SUBJECTS.map(ward => (
                    <div key={ward.name} className="relative">
                      {activeWard === ward.name && (
                        <div className="absolute -left-1.5 top-1/2 -translate-y-1/2 w-1 h-12 bg-primary rounded-full z-10"></div>
                      )}
                      <SubjectCard 
                        subject={ward.name}
